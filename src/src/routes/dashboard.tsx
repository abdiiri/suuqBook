        ...
        <>
          ...
          {role === "employee" && (
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <StatCard label="Your Commission Today" value={ksh(data.stats.myCommissionToday)} icon={Wallet} tone="green" />
              <StatCard label="Your Commission (30 days)" value={ksh(data.stats.myCommissionWindow)} icon={Wallet} tone="blue" />
            </div>
          )}
          ...
          {(role === "business_owner" || role === "super_admin") && (
            <div className="card-elevated mt-5 p-5">
              <div className="flex items-center justify-between">
                <h2 className="text-base font-semibold">Employee Commissions</h2>
                <span className="text-sm font-semibold text-primary">{ksh(data.stats.teamCommissionToday)} today</span>
              </div>
              {data.commissionByEmployee.length === 0 ? (
                <p className="mt-3 text-sm text-muted-foreground">
                  No employee commissions yet. Set a commission rate in{" "}
                  <Link to="/settings" className="font-medium text-primary hover:underline">
                    Settings
                  </Link>{" "}
                  so it's calculated automatically on every sale your employees make.
                </p>
              ) : (
                <>
                  <ul className="mt-4 divide-y divide-border">
                    {data.commissionByEmployee.map((c) => (
                      <li key={c.employeeId} className="flex items-center justify-between py-2.5 text-sm">
                        <span className="font-medium">{c.name}</span>
                        <span className="font-semibold">{ksh(c.amount)}</span>
                      </li>
                    ))}
                  </ul>
                  <p className="mt-3 text-xs text-muted-foreground">
                    Total owed to employees over the last 30 days: {ksh(data.stats.teamCommissionWindow)}
                  </p>
                </>
              )}
            </div>
