...
export const inviteEmployee = createServerFn({ method: "POST" })
  ...
  .validator((data: { email: string; password: string; fullName: string; phone: string; commissionRate: number }) => data)
  .handler(async ({ data, context }) => {
    ...
    const commissionRate = Math.min(100, Math.max(0, Number(data.commissionRate) || 0));
    ...
    await supabaseAdmin
      ...
      .update({
        owner_id: ownerId,
        business_name: callerProfile?.business_name ?? "",
        phone: data.phone,
